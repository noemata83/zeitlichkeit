        ...
        return (            <div>
                <FormControl className={classes.formControl}>
                    <InputLabel htmlFor='project_select'>Project</InputLabel>
                        ...
                        >{projectlist}</Select>
                    <TaskAutosuggest updateTaskInput={this.updateTaskInput} task={this.state.task} project={this.state.project || ''} />
                    <div className={classes.Timer}>
                        {this.state.timing ? this.displayTimer(this.state.counter) : "00:00:00" }
                    </div>
                    <Button variant="raised" color="secondary" onClick={() => this.state.timing ? this.handleStop() : this.handleStart()}>{this.state.timing ? <Stop /> : <PlayArrow />}</Button>
                </FormControl>
            </div>
        );
    }
